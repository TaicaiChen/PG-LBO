    ...
    def forward(self, *inputs: Tensor, validation: bool = False, m: Optional[float] = None, M: Optional[float] = None):
        ...
        if self.require_ys:
            ...
            if len(inputs) == 3:
                ...
                print('ys raise')
                raise ValueError(f"ys Can only have two tensor groups in batch, x and y but len(batch)={len(inputs)}")
        elif len(inputs) == 2:
            X, id = inputs[0], inputs[1]
        elif len(inputs) == 3:  # e.g. validation step in semi-supervised setup but we have targets that we do not use
            X, id, Y = inputs[0], inputs[1], inputs[2]
        else:
            raise ValueError(f"Can only have two tensor groups in batch, x and y but len(batch)={len(inputs)}")
        
        assert X.shape[0] == id.shape[0], (X.shape, id.shape)
        x = X[id == 1]
