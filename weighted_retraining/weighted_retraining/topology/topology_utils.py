...
def get_gp_data(data_scores, data_imgs, n_best, n_rand):
    if n_best > 0 and n_rand > 0 and (n_best + n_rand) < len(data_scores):
        sorted_idx = np.argsort(-data_scores)
        best_idx = sorted_idx[:n_best]
        rand_idx = sorted_idx[np.random.choice(list(range(n_best + 1, len(data_scores))), n_rand, replace=False)]
        ...
        cat_idx = np.arange(len(scores_best_cat_rand))
        cat_shuffled_idx = np.random.choice(cat_idx, len(cat_idx))
        scores_best_cat_rand = scores_best_cat_rand[cat_shuffled_idx].reshape((-1, 1))
        data_best_cat_rand = data_best_cat_rand[cat_shuffled_idx]
        return data_best_cat_rand, scores_best_cat_rand, cat_shuffled_idx
    return data_imgs, data_scores, np.arange(len(data_scores))


def get_seed_encodings(model, data_scores, data_imgs,
                         n_best, n_rand, tkwargs: Dict[str, Any],
                         bs=5000, return_inds: bool = False):

    n_batches = int(np.ceil(len(data_imgs) / bs))

    if (n_best + n_rand) > 0 and (n_best + n_rand) < len(data_scores):
        # do not encode all data, it's too long, only encode the number of points needed (w.r.t. n_best+n_rand)
    ...
    y = scores_best_cat_rand if (n_best + n_rand) > 0 and (n_best + n_rand) < len(data_scores) else data_scores
    y = y.reshape((-1, 1))
    
    if (n_best + n_rand) > 0 and (n_best + n_rand) < len(data_scores):
        if return_inds:
            inds = np.concatenate([best_idx, rand_idx])[cat_shuffled_idx]
            return X, y, inds
        else:
            return X, y
    else:
        if return_inds:
            inds = np.arange(len(data_scores))
            return X, y, inds
        else:
            return X, y
