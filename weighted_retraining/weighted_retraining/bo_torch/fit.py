...
def fit_gpytorch_torch_two(
        mll: MarginalLogLikelihood,
        mll0: MarginalLogLikelihood,
        bounds: Optional[ParameterBounds] = None,
        optimizer_cls: Optimizer = Adam,
        options: Optional[Dict[str, Any]] = None,
        track_iterations: bool = True,
    ...
    if exclude is not None:
        ...
        mll0_params = [
            t for p_name, t in mll0.named_parameters() if p_name not in exclude
        ]
    else:
        mll_params = list(mll.parameters())
        mll0_params = list(mll0.parameters())

    optimizer = optimizer_cls(
        params=[{"params": mll_params + mll0_params}],
        **_filter_kwargs(optimizer_cls, **optim_options),
    )
    ...
    if hasattr(mll0, "named_parameters_and_constraints"):
        for param_name, _, constraint in mll0.named_parameters_and_constraints():
            if constraint is not None and not constraint.enforced:
                bounds_[param_name] = constraint.lower_bound, constraint.upper_bound
    # update with user-supplied bounds (overwrites if already exists)
    if bounds is not None:
    ...
    while not stop:
        ...
        with gpt_settings.fast_computations(log_prob=approx_mll):
            ...
            output0 = mll0.model(*train_inputs)
            # we sum here to support batch mode
            args0 = [output0, train_targets] + _get_extra_mll_args(mll0)
            loss1 = -mll0(*args0).sum()
            
            loss2 = torch.abs(output.variance - output0.variance).mean()
        ...
        for n, p in mll0.named_parameters():
            if torch.isnan(p.grad).sum() > 0 or torch.isinf(p.grad).sum() > 0:  # or (p.grad.abs() < 1e-14).sum() > 0:
                # print('================ After Clipping ================')
                # print(n)
                # print(p.grad)
