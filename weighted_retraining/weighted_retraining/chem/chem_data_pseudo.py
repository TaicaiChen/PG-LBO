...
from weighted_retraining.weighted_retraining.chem.chem_utils import standardize_smiles, penalized_logP, QED_score, rdkit_quiet
...
class WeightedMolTreeFolder(MolTreeFolder):
    ...
    def __init__(self, prop, property_dict, data_weighter, isPseudo: bool = False, n_init_points: Optional[int] = None, *args, **kwargs):
...
class WeightedJTNNDataset(pl.LightningDataModule):
    ...
    def __init__(self, hparams, data_weighter, isPseudo=False):
        super().__init__()
        if isPseudo:
            self.train_path = hparams.pseudo_train_path
            self.val_path = None
            self.batch_size = int(hparams.pseudo_beta * hparams.batch_size)
        else:
    ...
    def add_model_specific_args(parent_parser):
        ...
        data_group.add_argument("--pseudo_train_path", required=False, default=None)
        data_group.add_argument("--pseudo_val_path", required=False, default=None)
        # 一个 mini batch 中伪标签数据与标签数据的比例
        data_group.add_argument("--pseudo_beta", required=False, type=float, default=0.5, help="in a batch use how many pseudo label data to train")
    ...
    def setup(self, stage=None, n_init_points: Optional[bool] = None):
        ...
        self.train_dataset = WeightedMolTreeFolder(
            ...
            predict_target=self.predict_target or self.use_ssdkl,
        )
    ...
    def specific_setup(self):
        if (self.predict_target or self.use_ssdkl):
            ...
            if self.val_dataset is not None:
                ...
                self.validation_m, self.validation_M = None, None
    ...
    def append_pseudo_data(self, smiles_new, z_prop, append_all_data: bool = None):
        dset = self.train_dataset
        ...
        task_num = len(smiles_new)//10 + 1
        with concurrent.futures.ProcessPoolExecutor(max_workers=10) as executor:
            futures = [executor.submit(append_data, smiles_new[i*task_num:(i+1)*task_num], z_prop[i*task_num:(i+1)*task_num], vocab_set) for i in range(10)]
            try:
                for future in concurrent.futures.as_completed(futures):
                    result = future.result()
                    s_add.extend(result[0])
                    data_to_add.extend(result[1])
                    props_to_add.extend(result[2])
                    prop_dict.update(result[3])
    ...
    def train_dataloader(self):
        return DataLoader(self.train_dataset, collate_fn=custom_collate_fn2, batch_size=None)

    def val_dataloader(self):
        if self.val_dataset is not None:
            return DataLoader(
                self.val_dataset, collate_fn=custom_collate_fn2, batch_size=None
            )

def custom_collate_fn2(batch):
    return [batch]

def append_data(smiles, prop, vocab_set):
    rdkit_quiet()
    s_add = []
    prop_dict = {}
    data_to_add = []
    ...
    for s, prop in zip(smiles, prop):
        ...
        if v_set <= vocab_set:
            ...
            # Update property dict for later
            prop_dict[s_std] = prop
    return s_add, data_to_add, props_to_add, prop_dict
...
class CustomIterableDataset(IterableDataset):
    ...
    def __iter__(self):
        iterator1 = iter(self.dataset1)
        iterator2 = cycle(iter(self.dataset2))
        for data1 in iterator1:
            data2 = next(iterator2)
            yield data1, data2

 
class PseudoWeightedJTNNDataset(pl.LightningDataModule):
    """ Implements a weighted numpy dataset (used for shapes task) """

    def __init__(self):
        super().__init__()

    def setup(self, LabelDataModule: WeightedJTNNDataset, PseudoDataModule: WeightedJTNNDataset):
        # Make into tensor datasets
        self.train_dataset = CustomIterableDataset(LabelDataModule.train_dataset, PseudoDataModule.train_dataset)
        if LabelDataModule.val_dataset is not None:
            if PseudoDataModule.val_dataset is not None:
                self.val_dataset = CustomIterableDataset(LabelDataModule.val_dataset, PseudoDataModule.val_dataset)
            else:
                self.val_dataset = LabelDataModule.val_dataset
        else:
            self.val_dataset = None
