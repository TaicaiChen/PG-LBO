...
beta_ssdkl_loss=0.01

#-- Choose the pseudo data sampling you want to use --#
pseudo_sampling_type_ind=4
pseudo_sampling_type=("" "random" "cmaes" "ga" "noise")
pseudo_sampling_type_kws=("" "{'use_filter':False}" "{'n_best':100,'n_rand':50,'sigma':0.25,'use_bo':True,'use_filter':False}" "{'n_best':100,'n_rand':0,'sigma':0.1,'use_bo':True,'use_filter':False}" "{'n_best':100,'n_rand':0,'sigma':0.1,'use_bo':True,'use_filter':True}")
...
for seed in "${seed_array[@]}"; do
  ...
  # Run command
  cmd="python weighted_retraining/weighted_retraining/robust_opt_scripts/robust_opt_expr_pseudo.py \
        --seed=$seed $gpu \
        ...
        --lr $lr $semi_supervised $n_init_bo_points $use_pretrained \
        $use_pseudo \
