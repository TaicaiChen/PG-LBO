...
acq_func_opt_kwargs_s=("{'batch_limit':100}" "{'batch_limit':50,'maxiter':500}" "{'batch_limit':50,'maxiter':500}")
...
pseudo_sampling_type_kws=("" "{'use_filter':False}" "{'n_best':100,'n_rand':0,'sigma':0.25,'use_bo':False,'use_filter':False}" "{'n_best':100,'n_rand':0,'use_bo':False,'use_filter':False}" "{'n_best':100,'n_rand':50,'sigma':0.1,'use_bo':True,'use_filter':True}")
...
for seed in "${seed_array[@]}"; do
  ...
  # Run command
  cmd="python weighted_retraining/weighted_retraining/robust_opt_scripts/robust_opt_topology_pseudo.py \
          --seed=$seed \
          ...
          $semi_supervised $n_init_bo_points\
          $use_pseudo \
